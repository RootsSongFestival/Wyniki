const db = {
    ...
    hosts: { 1: "PL", 2: "DE", 3: "FR", 4: "IT", 5: "ES", 6: "SE", 7: "GB", 8: "NL", 9: "UA", 10: "FI" }, 
    cities: { 1: "Gdańsk", 2: "Berlin", 3: "Paryż", 4: "Rzym", 5: "Madryt", 6: "Sztokholm", 7: "Londyn", 8: "Amsterdam", 9: "Kijów", 10: "Helsinki" }, 
    
    // Tablica zostanie wypełniona automatycznie przez generator na dole pliku
    results: [], 
...
/* =======================================================
   ...
   Gdy skończysz testy, skasuj wszystko poniżej tej linii
======================================================= */

const allCountryIds = Object.keys(db.countries);
const managersList = ["Oskar", "Kamil", "Anna", "Piotr", "Zofia", "Adam", "Magda", "Krzysztof", "Ewa"];

db.editions.forEach(edition => {
    // Losujemy 25 unikalnych państw dla danej edycji
    let shuffledCountries = [...allCountryIds].sort(() => 0.5 - Math.random()).slice(0, 25);
    
    // Losujemy kolejność startową (RO) od 1 do 25 bez powtórzeń
    let runningOrders = Array.from({length: 25}, (_, i) => i + 1).sort(() => 0.5 - Math.random());

    for (let place = 1; place <= 25; place++) {
        let countryId = shuffledCountries[place - 1];
        let ro = runningOrders[place - 1];
        let points = 350 - (place * 12) + Math.floor(Math.random() * 15); // Im wyższe miejsce, tym więcej punktów
        ...
        db.results.push({
            ...
            ro: ro,
            countryId: countryId,
            artist: `Testowy Artysta ${Math.floor(Math.random() * 999)}`,
            song: `Tytuł Piosenki ${Math.floor(Math.random() * 999)}`,
            manager: managersList[Math.floor(Math.random() * managersList.length)],
            points: points,
